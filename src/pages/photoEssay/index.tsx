...
const PhotoEssayGallery: React.FC = () => {
  ...
  const handleLightboxClose = () => setLightboxState(prev => ({ ...prev, isOpen: false }));
  const handleLightboxPrevious = () => setLightboxState(prev => ({
    ...prev,
    currentIndex: Math.max(0, prev.currentIndex - 1),
    zoomLevel: 1
  }));
  const handleLightboxNext = () => setLightboxState(prev => ({
    ...prev,
    currentIndex: Math.min(filteredImages.length - 1, prev.currentIndex + 1),
    zoomLevel: 1
  }));
  const handleZoomIn = () => setLightboxState(prev => ({
    ...prev,
    zoomLevel: Math.min(3, prev.zoomLevel + 0.5),
    isZoomed: prev.zoomLevel >= 1
  }));
  const handleZoomOut = () => setLightboxState(prev => ({
    ...prev,
  ...
  return (
    <>
      ...
      {/* Ensure this div flexes properly within DashboardLayout */}
      <div className="flex-1 min-h-screen p-4 md:p-6 bg-background">
        {/* Filters */}
        <GalleryFilter
          filters={filters}
