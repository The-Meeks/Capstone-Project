...
const PresentationsGallery = () => {
  ...
  useEffect(() => {
    ...
    if (filters.subject) filtered = filtered.filter((p) => p.subject === filters.subject);
    if (filters.type) filtered = filtered.filter((p) => p.type === filters.type);
    if (filters.language) filtered = filtered.filter((p) => p.language === filters.language);
  ...
  const handleSlideChange = (slideNumber: number) => setCurrentSlide(slideNumber);
  ...
  return (
    <>
      ...
      <div className="min-h-screen bg-background flex flex-col">
        ...
        <main className="w-full p-4 sm:p-6 lg:p-8 pt-24 sm:pt-28 lg:pt-32">
